...
No Gatekeeper warning: Homebrew does not quarantine what a formula installs.
...
`throughline` sets Throughline up in `~/throughline-os` the first time and starts it; after that the same command starts it and says when a newer release is published. It installs nothing Gatekeeper checks — Homebrew does not quarantine what a formula installs — so there is no trip to System Settings. `brew uninstall throughline` removes the command and never touches your data in `~/.throughline-os`. The formula lives in [`homebrew-throughline`](https://github.com/SarthakPattnaik1/homebrew-throughline) and is generated by `scripts/homebrew.py` from the signed release manifest.
